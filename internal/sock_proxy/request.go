...
func (r *Request) Process() {
	...
	if !r.tcpGram.viaUPD { // tcp
		// answer bind addr
		if conn, err := net.DialTimeout("tcp", r.tcpGram.networkString(), time.Second*time.Duration(r.server.writeTimeout)); err != nil {
...
func (r *Request) transformUDP() {
	...
	buf := make([]byte, 65535)
	for {
		_ = r.UDPConn.SetReadDeadline(time.Now().Add(time.Second * time.Duration(r.server.readTimeout)))
		...
		if r.udpGram.clientAddr.IP.Equal(fromAddr.IP) && r.udpGram.clientAddr.Port == fromAddr.Port {
			...
			if exchange, ok := r.udpGram.UDPExchangeMap[r.udpGram.remoteAddr().String()]; !ok {
				...
				logging.Log.Infof("[%s]connect to: %s:%d", "udp", target, r.tcpGram.port)
			} else {
				exchange.Delay(60)
			}
			r.udpGram.udpMutex.Unlock()

			_ = r.UDPConn.SetWriteDeadline(time.Now().Add(time.Second * time.Duration(r.server.writeTimeout)))

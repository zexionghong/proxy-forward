...
func NewServer() *Server {
	addr, err := net.ResolveTCPAddr("tcp", config.RuntimeViper.GetString("sock_proxy_server.port"))
	if err != nil {
	...
	return &Server{
		listener:     l,
		mutex:        sync.Mutex{},
		enableUPD:    config.RuntimeViper.GetBool("sock_proxy_server.udp_support"),
		readTimeout:  config.RuntimeViper.GetInt("sock_proxy_server.read_timeout"),
		writeTimeout: config.RuntimeViper.GetInt("sock_proxy_server.write_timeout"),
	}
}

// Serve Run
func (s *Server) Run() error {
	logging.Log.Infof("start server on: %s", s.listener.Addr().String())
	for {
